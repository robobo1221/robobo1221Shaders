...
#define ozoneNumberDensity     (airNumberDensity * ozoneConcentrationPeak)
...
#define sky_coefficientOzone     (ozoneCrossSection * (ozoneNumberDensity * 1e-6)) // ozone cross section * (ozone number density * (cm ^ 3))
#define sky_coefficientMie      vec3(3.0000e-6, 3.0000e-6, 3.0000e-6) // Should be >= 2e-6

#define sky_inverseScaleHeights     (1.0 / sky_scaleHeights)
#define sky_scaledPlanetRadius      (sky_planetRadius * sky_inverseScaleHeights)
#define sky_atmosphereRadius        (sky_planetRadius + sky_atmosphereHeight)
#define sky_atmosphereRadiusSquared (sky_atmosphereRadius * sky_atmosphereRadius)
